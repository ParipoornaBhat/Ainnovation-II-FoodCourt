  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      if (result?.error === "CredentialsSignin") {
        alert("Invalid team name or password. Please try again.");
      } else if (result?.ok) {
        document.cookie = [
      ...
      <Card className="shadow-lg">
        <CardHeader className="text-center space-y-4">
          ...
          <div>
            ...
            <CardDescription>Access your team's food ordering portal</CardDescription>
          </div>
        </CardHeader>
        <CardContent>
          ...
          <div className="mt-6 text-center space-y-4">
            ...
            <div className="pt-4 border-t">
              <p className="text-sm text-muted-foreground">
                Need admin access?{" "}
                <Link href="/admin/login" className="text-primary hover:text-primary/80 transition-colors duration-200">
                  Admin Login
                </Link>
              </p>
            </div>
