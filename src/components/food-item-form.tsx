...
import type {
	FoodItemFormProps,
	AddFoodItemPageProps,
} from "@/app/admin/food/add/page";
...
export function FoodItemForm({ item, onSubmit, onCancel }: FoodItemFormProps) {
	const [formData, setFormData] = useState<AddFoodItemPageProps>({
		name: item?.name || "",
	...
	const addCustomRestriction = () => {
		if (
			...
			!formData.restrictions.includes(customRestriction.trim())
		) {
			setFormData((prev) => ({
				...prev,
					...
					<div className="space-y-2">
						...
						<Input
							...
							onChange={(e) =>
								setFormData({
									...formData,
									availableQty: Number.parseInt(e.target.value) || 0,
								})
							}
							placeholder="0"
							required
